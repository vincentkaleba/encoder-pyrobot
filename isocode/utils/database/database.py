...
class Database:
    ...
    def new_user(self, id):
        return dict(
            ...
            crf=22,
            resize=False,
            subs_id=0,  # ID du flux de sous-titres à extraire
        )
    ...
    async def get_or_create_user(self, id):
    ...
    async def update_user_setting(self, user_id, setting_name, value):
        ...
        if user:
            user[setting_name] = value
            await self.col.update_one({'id': int(user_id)}, {'$set': user})

    async def get_user_setting(self, user_id, setting_name):
        user = await self.col.find_one({'id': int(user_id)})
        if user:
            return user.get(setting_name)